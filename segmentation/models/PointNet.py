...
def feature_transform_reguliarizer(trans):
    d = trans.size()[1]
    I = torch.eye(d)[None, :, :]
    if trans.is_cuda:
        I = I.cuda()
...
class get_model(nn.Module):
    ...
    def load_model_from_ckpt(self, ckpt_path):
        ...
        if incompatible.missing_keys:
            print_log('missing_keys', logger='PointNetPartSeg')
            print_log(
                get_missing_parameters_message(incompatible.missing_keys),
                logger='PointNetPartSeg'
            )
        if incompatible.unexpected_keys:
            print_log('unexpected_keys', logger='PointNetPartSeg')
            print_log(
    ...
    def forward(self, x):
        ...
        x = F.log_softmax(x.view(-1, self.cls_dim), dim=-1)
        x = x.view(batchsize, n_pts, self.cls_dim)
        return x
